...
/**
 * For lifecycle create tag
 * only usage when create hook only call other method
 * usage: @Create function() {}
 */
export function Created(target: any, propertyKey: string): void {
  mergeToComponent(target, propertyKey, 'created', false, false);
}
